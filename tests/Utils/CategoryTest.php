...
class CategoryTest extends KernelTestCase
{
        ...
        foreach($tested_classes as $class)
        {
            if($class == 'CategoryTreeAdminOptionList') {
                ...
                $this->$name = $this->getMockBuilder('App\Utils\\'.$class)
                    ...
                    ->setMethods(['buildTree', 'getCategoryList']) // if no, all methods return null unless mocked
                    ->getMock();
            } else {
                $name = 'mocked'.$class;
                $this->$name = $this->getMockBuilder('App\Utils\\'.$class)
    ...
    {
        yield ['<ul class="fa-ul text-left"><li><i class="fa-li fa fa-arrow-right"></i>Toys<a href="/admin/su/edit-category/2"> Edit</a> <a onclick="return confirm(\'Are you sure?\');"
    href="/admin/su/delete-category/2">Delete</a></li></ul>',
